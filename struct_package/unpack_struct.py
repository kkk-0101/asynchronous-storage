...
from crypto.ABE1.ABE.msp import MSP
...
def tyke_2(m_bytes):
    ...
    if debug:
        print("---Read database successfully---", m)
    try:
        mm = Broadcast_decryption(3, m)
        if debug:
            print("The user successfully received the message---", mm.decode())
        return mm.decode()
    except:
...
def tyke_4(m_bytes):
    ...
    if debug:
        print("---The user successfully received the message")
    try:
        attr_list = ['ONE', 'TWO', 'THREE']
        mm = decrypt(attr_list, m)
        if debug:
            print("---Successfully received message---", mm.decode())
        return mm.decode()
    except:
        print("Decryption failure")
...
def tyke_6(m_bytes):
    if debug:
        print("---The user successfully received the message", m_bytes)
    if debug:
        print("---Successfully received message---", m_bytes.decode())
    return m_bytes.decode()


def _unpack(i, b, chain_put, db_put):
    ...
    # print("tyke---",tyke)
    fields_len_bytes = buf.read(4)
    fields_len, = struct.unpack("<i", fields_len_bytes)
    fields_bytes = buf.read(fields_len)
    ...
    if tyke == 1:
        db_put((i, key_bytes, m_bytes))
        chain_put(fields_bytes)
        if debug:
            print(str(i), "---Successfully saved to database---", m_bytes)
    elif tyke == 2:
        db_put((i, key_bytes, tyke))

    elif tyke == 3:
        db_put((i, key_bytes, m_bytes))
        chain_put(fields_bytes)
        if debug:
            print(str(i), "---Successfully saved to database")
    elif tyke == 4:
        db_put((i, key_bytes, tyke))
